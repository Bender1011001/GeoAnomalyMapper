...
def classify_supervised(feature_paths: List[str],
    ...
    classifier, scaler = train_supervised_model(
        positive_features, negative_features, n_estimators, random_state
    )
...
def generate_engineered_features(feature_paths: List[str],
    ...
    """
    Generate engineered features from base rasters and return updated feature paths.
    ...
    if gravity_data is not None or magnetic_data is not None:
        # Stack features and get new paths
        ...
        logger.info(f"  ✅ Generated {len(engineered_paths)} engineered features")
