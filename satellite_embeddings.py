...
from json_utils import dumps_strict_json, to_strict_jsonable
...
def _json_safe(obj):
    return to_strict_jsonable(obj)


def _write_summary_json(path: Path, summary: Dict) -> None:
    path = Path(path)
    _ensure_parent(path)
    path.write_text(dumps_strict_json(summary, indent=2) + "\n", encoding="utf-8")
...
def inspect_embedding_raster(
    ...
    *,
    ...
    if summary_out:
        ...
        _write_summary_json(summary_out, summary)
...
def cluster_embedding_anomalies(
    ...
    *,
    ...
    # --- Pass 1: subsample pixels for KMeans fit (AUDIT FIX: reservoir sampling) ---
    # The old sequential buffer fill only sampled from early (Northern) blocks
    # of large GeoTIFFs, biasing K-Means toward one geographic region.
    # Reservoir sampling gives every pixel equal probability of inclusion.
    sample_count = 0
    with rasterio.open(raster_path) as src:
        indexes = _embedding_band_indexes(src)
        ...
        reservoir = np.zeros((max_pixels_for_fit, len(indexes)), dtype=np.float32)
        ...
        for _, window in src.block_windows(1):
            ...
            if sample_count < max_pixels_for_fit:
                fill_len = min(n, max_pixels_for_fit - sample_count)
                reservoir[sample_count : sample_count + fill_len] = valid_vecs[:fill_len]

                if n > fill_len:
                    rem_n = n - fill_len
                    rem_vecs = valid_vecs[fill_len:]
                    probs = max_pixels_for_fit / (sample_count + fill_len + np.arange(1, rem_n + 1))
                    mask = np.random.rand(rem_n) < probs
                    n_replace = np.count_nonzero(mask)
                    if n_replace > 0:
                        replace_idx = np.random.randint(0, max_pixels_for_fit, size=n_replace)
                        reservoir[replace_idx] = rem_vecs[mask]
            else:
                probs = max_pixels_for_fit / (sample_count + np.arange(1, n + 1))
                mask = np.random.rand(n) < probs
                n_replace = np.count_nonzero(mask)
                if n_replace > 0:
                    replace_idx = np.random.randint(0, max_pixels_for_fit, size=n_replace)
                    reservoir[replace_idx] = valid_vecs[mask]
    ...
    fit_data = reservoir[:min(sample_count, max_pixels_for_fit)]
        ...
        with rasterio.open(raster_path) as src:
            ...
            with rasterio.open(anomaly_out, "w", **profile) as dst:
                ...
                for _, window in src.block_windows(1):
                    block, invalid = _read_embedding_block(src, indexes, window)
                    b_flat = block.reshape(len(indexes), -1).T  # (N, 64)
                    # Invalid pixels are NaN-filled; sklearn rejects NaN input,
                    # so zero them for predict and mask the scores afterwards.
    ...
    if summary_out:
        _write_summary_json(Path(summary_out), summary)
    return summary
