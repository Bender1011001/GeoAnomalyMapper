...
def _normalize_result_anomaly_counts(result):
    """Ensure both anomaly-count keys are present and consistent on a result."""
    if not isinstance(result, dict):
        return 0
    count = _get_anomaly_count(result)
    result["anomaly_count"] = count
    result["anomalies_detected"] = count
    return count
...
def worker_wrapper(args):
    target, credentials, resolution = args
    ...
    try:
        ...
        result["elapsed_seconds"] = (datetime.now() - target_start).total_seconds()
        _normalize_result_anomaly_counts(result)
        return result
        ...
        return {
            ...
            "elapsed_seconds": (datetime.now() - target_start).total_seconds(),
        }
            ...
            with open(report_path, "r", encoding="utf-8") as f:
                ...
                for r in prev_report.get("results", []):
                    ...
                    if status not in ("error", "failed"):
                        _normalize_result_anomaly_counts(r)
                        existing_results[r.get("target_name", r.get("target"))] = r
        except Exception as e:
            logger.warning(f"Failed to read existing report: {e}")

    args_list = []
    for target in targets:
        if target["name"] in existing_results:
            results.append(existing_results[target["name"]])
            logger.info(f"Skipping already completed target: {target['name']}")
        else:
            args_list.append((target, credentials, resolution))
    
    with ProcessPoolExecutor(max_workers=8) as executor:
        future_to_target = {executor.submit(worker_wrapper, args): args[0]["name"] for args in args_list}
        ...
        for i, future in enumerate(as_completed(future_to_target), 1):
            ...
            anomalies = _normalize_result_anomaly_counts(result)
            ...
            if result.get("status") == "error":
                logger.error(f"  >> [{i}/{len(targets)}] FAILED: {name}: {result.get('error')}")
            else:
                status_icon = "ANOMALY" if anomalies > 0 else "CLEAN"
                logger.info(f"  >> [{i}/{len(targets)}] [{status_icon}] {name}: "
