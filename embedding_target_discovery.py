    ...
    *,
    ...
    list of dict
        Ranked target list. Each entry is compatible with PHASE_X_TARGETS:
        {
            ...
            "cluster_score": float,
            "spatial_score": float,
            "temporal_score": float,  # None if temporal not run
    ...
    try:
        ...
        for w, layer in zip(available_weights, available_layers):
            if layer.shape != (H, W):
                raise RuntimeError(
                    f"[Cap 7] Anomaly layer shape {layer.shape} does not match primary "
                    f"raster grid {(H, W)}; all inputs must share the same export grid."
                )
        ...
        for rank, (row, col, score) in enumerate(peaks[:max_targets], start=1):
            ...
            cl_score = float(cluster_arr[row, col]) if cluster_arr is not None else None
            sp_score = float(spatial_arr[row, col]) if spatial_arr is not None else None
            tm_score = float(temporal_arr[row, col]) if temporal_arr is not None else None
            ...
            targets.append({
                ...
                "cluster_anomaly_score": round(cl_score, 5) if cl_score is not None and np.isfinite(cl_score) else None,
                "spatial_anomaly_score": round(sp_score, 5) if sp_score is not None and np.isfinite(sp_score) else None,
                "temporal_variance_score": round(tm_score, 5) if tm_score is not None and np.isfinite(tm_score) else None,
                "pixel_row": row,
                "pixel_col": col,
                "region": region_name,
                "source": "embedding_auto_discovery",
...
def main() -> None:
    ...
    if args.targets_json:
        ...
        args.targets_json.write_text(
            dumps_strict_json({"region": args.region, "targets": targets}, indent=2) + "\n",
            encoding="utf-8",
        )
        logger.info(f"Target list written to {args.targets_json}")
    else:
        print(dumps_strict_json({"region": args.region, "targets": targets}, indent=2))
